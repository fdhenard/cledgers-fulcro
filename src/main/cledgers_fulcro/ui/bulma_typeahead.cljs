...
(defsc TypeaheadComponent
  ...
  (let [_ (println "rendering TypeaheadComponent")
        ...
        set-matches!
        (fn [in]
          (comp/set-state! this {:matches in}))
        set-textbox-value!
        (fn [in]
          (comp/set-state! this {:textbox-value in}))
        get-textbox-value
        (fn []
          (comp/get-state this :textbox-value))
        should-expand-dropdown? (not= textbox-value selection-value)]
    (dom/div
     {:className (cls-set->str
                  #{:dropdown
                    (when should-expand-dropdown? :is-active)})}
     (dom/div
      :.dropdown-trigger
